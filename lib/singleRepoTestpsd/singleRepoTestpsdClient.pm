package singleRepoTestpsd::singleRepoTestpsdClient;
...
singleRepoTestpsd::singleRepoTestpsdClient

=head1 DESCRIPTION


A KBase module: singleRepoTestpsd
...
sub new
{
    ...
    my $self = {
	client => singleRepoTestpsd::singleRepoTestpsdClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$workspace_name is a singleRepoTestpsd.workspace_name
$contigset_id is a singleRepoTestpsd.contigset_id
$return is a singleRepoTestpsd.CountContigsResults
...
# Authentication: required
	...
	method => "singleRepoTestpsd.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "singleRepoTestpsd.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for singleRepoTestpsd::singleRepoTestpsdClient\n";
    }
    if ($sMajor == 0) {
        warn "singleRepoTestpsd::singleRepoTestpsdClient version is $svr_version. API subject to change.\n";
    }
}
...
package singleRepoTestpsd::singleRepoTestpsdClient::RpcClient;
