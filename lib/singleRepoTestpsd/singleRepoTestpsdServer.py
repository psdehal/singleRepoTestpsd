...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'singleRepoTestpsd'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from singleRepoTestpsd.singleRepoTestpsdImpl import singleRepoTestpsd
impl_singleRepoTestpsd = singleRepoTestpsd(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['singleRepoTestpsd.count_contigs_async'] = ['singleRepoTestpsd', 'count_contigs']
async_check_methods['singleRepoTestpsd.count_contigs_check'] = ['singleRepoTestpsd', 'count_contigs']
sync_methods['singleRepoTestpsd.count_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'singleRepoTestpsd'
        ...
        self.rpc_service.add(impl_singleRepoTestpsd.count_contigs,
                             name='singleRepoTestpsd.count_contigs',
                             types=[basestring, basestring])
        self.method_authentication['singleRepoTestpsd.count_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "singleRepoTestpsd but no authentication header was passed"
                            raise err
